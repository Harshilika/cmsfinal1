    ...
    if (is_array($_POST['results'])) { // Check if results is an array
        ...
        foreach ($_POST['results'] as $student_id => $result) {
            ...
            $insert_sql = "INSERT INTO results (student_id, course_id, exam_type, score) VALUES (?, ?, ?, ?)
                           ON DUPLICATE KEY UPDATE score = score"; // Prevent updating existing scores
                                        ...
                                        <?php if (!empty($students)): ?>
                                            <?php foreach ($students as $student): ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <?php if (isset($existing_results[$student['id']])): ?>
                                                            <input type="number" name="results[<?php echo $student['id']; ?>]" class="form-control" value="<?php echo htmlspecialchars($existing_results[$student['id']]); ?>" readonly>
                                                        <?php else: ?>
                                                            <input type="number" name="results[<?php echo $student['id']; ?>]" class="form-control" value="" min="0" max="100" required>
                                                        <?php endif; ?>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
